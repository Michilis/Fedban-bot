...
from db import create_federation, delete_federation, get_fed_info, get_feds_by_owner, add_fed_admin, remove_fed_admin, add_banned_user, remove_banned_user
...
async def set_unset_fed_log(client: Client, message: Message):
    ...
    await conn.execute('UPDATE federations SET log_group_id = $1 WHERE fed_id = $2', log_group_id, fed_id)
...
async def join_fed(client: Client, message: Message):
    ...
    await conn.execute('UPDATE federations SET chat_ids = array_append(chat_ids, $1) WHERE fed_id = $2', message.chat.id, fed_id)
...
async def leave_fed(client: Client, message: Message):
    ...
    await conn.execute('UPDATE federations SET chat_ids = array_remove(chat_ids, $1) WHERE fed_id = $2', message.chat.id, fed_id)
...
async def fpromote(client: Client, message: Message):
    ...
    if not fed_id:
        return await message.reply_text("This group is not in any federation.")
    if not await is_fed_owner_or_admin(message.from_user.id, fed_id):
        return await message.reply_text("Only federation owners and admins can promote.")
...
async def fdemote(client: Client, message: Message):
    ...
    if not fed_id:
        return await message.reply_text("This group is not in any federation.")
    if not await is_fed_owner_or_admin(message.from_user.id, fed_id):
        return await message.reply_text("Only federation owners and admins can demote.")
    user_id, _ = extract_user_and_reason(message)
    if not user_id:
